-- ============================================================
-- אנליטיקס Supabase – להעתקה ל-SQL Editor (הכל בקובץ אחד)
-- העתק את כל התוכן מהשורה הבאה עד סוף הקובץ והדבק ב-Supabase → SQL Editor → Run
-- ============================================================

-- 1. מחיקת הפונקציה הישנה (חובה לפני שינוי סוג ההחזרה)
DROP FUNCTION IF EXISTS public.get_analytics_summary(integer);

-- 2. יצירת פונקציית הסיכום המעודכנת – עם כל המדדים החדשים
CREATE OR REPLACE FUNCTION public.get_analytics_summary(days_back INT DEFAULT 7)
  ...
  -- 3. אינדקס לביצועים (אופציונלי – אם יש הרבה שורות)
  CREATE INDEX IF NOT EXISTS idx_analytics_event_type_created 
  ON public.analytics_events(event_type, created_at DESC);

-- 4. משתמשים אונליין (זמן אמת) – מבקרים עם אירוע ב-X דקות אחרונות
