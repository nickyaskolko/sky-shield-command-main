...
import { Play, Info, Heart, Settings, BarChart3, BookOpen, Trophy, LogIn, LogOut, User, Gift, ShoppingBag, MessageSquare } from 'lucide-react';
...
export function MainMenu({ highScore, onStartGame, hasSavedGame, onContinueGame, onStartStory, onHowToPlay, onOpenSettings, onOpenStats, onOpenChallenges, onOpenDailyReward, onOpenShop, userEmail, onOpenAuth, onSignOut, canClaimDailyReward }: MainMenuProps) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center z-30 p-4 overflow-y-auto" dir="rtl">
      ...
      >
        {/* Hero block – כותרת + תיאור (רגע הדגשה) */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.4 }}
          className="mb-4 rounded-2xl border-2 border-amber-400/40 bg-gradient-to-b from-amber-500/15 to-transparent px-6 py-5 shadow-[0_0_30px_rgba(251,191,36,0.15)] ring-2 ring-amber-400/20 shrink-0"
        >
          <motion.h1
            initial={{ y: -12, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.15, duration: 0.35 }}
            className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-white drop-shadow-[0_0_16px_hsla(185,100%,50%,0.7)] drop-shadow-[0_0_32px_hsla(185,100%,50%,0.4)] [text-shadow:0_0_24px_hsla(185,100%,70%,0.8)] tracking-tight text-center"
          >
            {t('gameTitle')}
          </motion.h1>
          <motion.p
            ...
            className="text-amber-100 text-base sm:text-lg font-semibold mt-3 text-center drop-shadow-md [text-shadow:0_1px_4px_rgba(0,0,0,0.4)]"
          >
            משחק הגנת שמיים טקטי
          </motion.p>
        </motion.div>

        {/* Menu Card – מופיע אחרי הכותרת והתיאור */}
        <motion.div
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.5, duration: 0.25 }}
          className="w-full bg-game-panel/90 backdrop-blur-md rounded-xl px-4 py-3 sm:px-5 sm:py-4 border border-game-accent/30 shadow-lg"
        >
          <div className="space-y-2">
            ...
            <div className="flex justify-center">
              <div className="flex items-center gap-2" title="משפיע על התקציב והמורל ההתחלתיים">
                <Label className="text-game-text-dim text-sm shrink-0">קושי:</Label>
                <Select value={difficulty} onValueChange={(v: 'easy' | 'normal' | 'hard') => setDifficulty(v)}>
                  <SelectTrigger className="w-[100px] bg-game-bg/60 border-game-accent/20 text-game-text h-9">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-2 pt-0.5">
              ...
              {onOpenDailyReward && (
                ...
                >
                  ...
                  פרס יומי
                  {canClaimDailyReward && (
                    <span className="absolute -top-0.5 -left-0.5 h-3 w-3 rounded-full bg-amber-500 text-game-panel text-[10px] flex items-center justify-center font-bold">!</span>
                  )}
                </Button>
              )}
              {onOpenSettings && (
        ...
        >
          ...
          >
            <Button
              variant="outline"
              size="default"
              className="border-2 border-amber-400 bg-amber-500/30 text-amber-50 hover:bg-amber-500/50 hover:border-amber-300 hover:text-white font-bold text-base px-6 h-11 shadow-xl shadow-amber-500/25"
              asChild
            >
              <a
